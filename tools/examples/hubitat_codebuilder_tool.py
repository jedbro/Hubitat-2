...
hhs.setFormatter(HubitatCodeBuilderLogFormatter(
    error_beep=True, debug_color=Fore.CYAN, default_color=Fore.MAGENTA))
...
def main():
    ...
    base_repo_url = 'https://github.com/markus-li/Hubitat/blob/' + \
        branch_name+'/drivers/expanded/'
    base_raw_url_root = 'https://raw.githubusercontent.com/markus-li/Hubitat/' + \
        branch_name+'/'
    base_raw_repo_url = base_raw_url_root + 'drivers/expanded/'
    app_raw_repo_url = base_raw_url_root + 'apps/expanded/'
    ...
    pm = HubitatPackageManagerTool("Markus", "2.2.2",
    ...
    driver_files = [
        ...
        {'id': 1057, 'file': 'testing-get-driver-runtime-data.groovy',
            'version': 'v0.1.0.MMDD' + version_suffix},
        ...
        {'id': 1858, 'id_2': 0, 'id_3': 0, 'file': 'zigbee-ikea-tradfri-on-off-switch.groovy', 'version': default_zigbee_version,
    ...
    driver_files_private = [
        ...
        {'id': 547, 'id_3': 341, 'file': 'testing-bare-minimum-driver.groovy',
            'version': 'v0.1.0.MMDD' + version_suffix},
    ...
    driver_files_active = [
        ...
        {'id': 1409, 'id_3': 0, 'file': 'dashboard-background-image.groovy',
            'version': 'v0.1.0.MMDD' + version_suffix},
    ...
    driver_files_active = getExpandedDriverList(
        driver_files_active, driver_files)
    driver_files_active_2 = getExpandedDriverList(
        driver_files_active_2, driver_files)
    driver_files_active_3 = getExpandedDriverList(
        driver_files_active_3, driver_files)

    driver_files_private_active = getExpandedDriverList(
        driver_files_private_active, driver_files_private)
    driver_files_private_active_2 = getExpandedDriverList(
        driver_files_private_active_2, driver_files_private)
    # print(driver_files)
    ...
    if(branch_name != 'release'):
        used_driver_list_2 = cb_2.expandGroovyFilesAndPush(
            driver_files_active_2, code_type='driver')
        used_driver_list_3 = cb_3.expandGroovyFilesAndPush(
            driver_files_active_3, code_type='driver')

    # The main target needs to be last to keep everything in a consistent state
    used_driver_list = cb.expandGroovyFilesAndPush(
        driver_files_active, code_type='driver')

    if(branch_name != 'release'):
        used_driver_list_private = cb_private.expandGroovyFilesAndPush(
            driver_files_private_active, code_type='driver')
        used_driver_list_private_2 = cb_private_2.expandGroovyFilesAndPush(
            driver_files_private_active_2, code_type='driver')
    print(used_driver_list[1890])
    ...
    for d in sorted_driver_list:
        ...
        if(d['name'].startswith('Zigbee - ')):
            ...
            if(newD['publish'] == True):
                zigbee_pkg = HubitatPackageManagerPackage(newD['name'], "Integrations",
                                                          "https://raw.githubusercontent.com/markus-li/Hubitat/release/packages/" +
                                                          newD['filestem'] +
                                                          ".json",
                ...
                if(branch_name == 'release'):
                    zigbee_pkg.buildManifest(
                        output="packages/" + newD['filestem'] + ".json", extraInput="packages/" + newD['filestem'] + ".json")
                else:
                    zigbee_pkg.buildManifest(
                        output="packages/" + newD['filestem'] + "-beta.json", extraInput="packages/" + newD['filestem'] + ".json")
        ...
        if(d['name'].startswith('Smartly')):
            ...
            if(newD['publish'] == True):
                smartly_pkg = HubitatPackageManagerPackage(newD['name'], "Utility",
                                                           "https://raw.githubusercontent.com/markus-li/Hubitat/release/packages/" +
                                                           newD['filestem'] +
                                                           ".json",
                                                           newD['comment'], isBeta=is_beta,
                ...
                pm.addPackage(smartly_pkg)
                print("Smartly package: " + newD['nameFull'])
                if(branch_name == 'release'):
                    smartly_pkg.buildManifest(
                        output="packages/" + newD['filestem'] + ".json", extraInput="packages/" + newD['filestem'] + ".json")
                else:
                    smartly_pkg.buildManifest(
                        output="packages/" + newD['filestem'] + "-beta.json", extraInput="packages/" + newD['filestem'] + ".json")
    ...
    if(len(used_driver_list) >= expected_num_drivers):
        ...
        if(branch_name == 'release'):
            ...
            for d in parent_drivers + child_drivers:
                output_file = "../Hubitat.wiki/Driver-List/[Driver-List]-Tasmota-" + \
                    d['wikiname'] + '.md'
                # print(output_file)
...
''')
        if(update_2nd_hub_drivers):
            ...
            my_driver_list_3 = [
                ...
                {'name': '', 'format': '  %(name)s\n'},
                {'name': 'DO NOT MODIFY ANYTHING EXCEPT "id", THIS FILE IS AUTOGENERATED!',
                    'format': '  # %(name)s\n'},
                {'name': '', 'format': '  %(name)s\n'},
                ...
                {'name': 'Tasmota - Specific Device Drivers',
                 ...
                 'items_format': [
                     "  {'id': %(id_2nd)d, 'original_id': %(id)d, 'name': '%(name)s'},\n", ]},
                {'name': ']', 'format': '%(name)s'}, ]
            cb.makeDriverListDoc(
                my_driver_list_3, output_file='private/tools/config/driver_list_2nd_hub.py')
        else:
            log.warn(
                "Can't update the 2nd hub drivers! Check other errors/warnings for details...")
    else:
        log.info(
            "SKIPPING making of the driver list file since we don't have enough drivers in the list...")
    ...
    app_files = [
        ...
        {'id': 289, 'id_2': 66, 'file': 'tasmota-device-manager.groovy',
            'required': True, 'oauth': False, 'publish': True},

        # Data API
        {'id': 641, 'id_2': 0, 'file': 'data-api.groovy',
            'required': True, 'oauth': True, 'publish': False},
        # Dashboard Apps
        {'id': 481, 'id_2': 97, 'file': 'smartly-enhanced-dashboard.groovy',
            'required': True, 'oauth': True, 'publish': False},
    ...
    app_files_private = [
        {'id': 353, 'id_3': 67, 'file': 'custom-lighting.groovy',
            'required': True, 'oauth': False, 'publish': False},
    ]
    ...
    if(branch_name != 'release'):
        used_app_list_private_3 = cb_private_3.expandGroovyFilesAndPush(
            app_files_private.copy(), code_type='app')
        used_app_list_2 = cb_2.expandGroovyFilesAndPush(
            filtered_app_files_2, code_type='app')

    # for i in range(100, 268):
    #    hhs_2.delete_code("app", i)

    # The main target needs to be last to keep everything in a consistent state
    used_app_list = cb.expandGroovyFilesAndPush(
        filtered_app_files.copy(), code_type='app')

    if(branch_name != 'release'):
        used_app_list_private = cb_private.expandGroovyFilesAndPush(
            app_files_private.copy(), code_type='app')
    ...
    if(branch_name == 'release'):
        ...
        t4he_pkg.buildManifest(output="packageManifest.json",
                               extraInput="packageManifestBeta.json")
    else:
        #t4he_pkg.buildManifest(output="packages/t4he-beta.json", extraInput="packages/t4he.json")
        t4he_pkg.buildManifest(
            output="packageManifestBeta.json", extraInput="packageManifest.json")
        # t4he_pkg.printJSON()
    ...
    if(repo_public.head.shorthand != repo_private.head.shorthand):
        repo_public.checkout(repo_public.lookup_reference(
            repo_public.lookup_branch(repo_private.head.shorthand).name))

    if(repo_public.head.shorthand == repo_private.head.shorthand):
        log.debug("The Hubitat-Public Repo has branch \"" +
                  repo_public.head.shorthand + "\" checked out.")
        ...
        # Public Apps
        repo_tool.copy_files_by_wildcard(
            repo_private_path + "/apps/expanded/tasmota-device-manager-expanded.groovy", repo_public_path + "/apps/expanded")

        # App Repo Manifests
        repo_tool.copy_files_by_wildcard(
            repo_private_path + "/packages/*.json", repo_public_path + "/packages")
        repo_tool.copy_files_by_wildcard(
            repo_private_path + "/packageManifest.json", repo_public_path)
        repo_tool.copy_files_by_wildcard(
            repo_private_path + "/packageManifest.json", repo_public_path + "/packages/t4he.json")
        repo_tool.copy_files_by_wildcard(
        ...
        # README
        repo_tool.copy_files_by_wildcard(
            repo_private_path + "/README.md", repo_public_path)

        if(repo_private.head.shorthand == "development"):
            # Examples of how to use the Codebuilder toolset
            repo_tool.copy_files_by_wildcard(
                repo_private_path + "/tools/hubitat_codebuilder_tasmota.py", repo_public_path + "/tools/examples")
            repo_tool.copy_files_by_wildcard(
                repo_private_path + "/tools/hubitat_codebuilder_tool.py", repo_public_path + "/tools/examples")
            repo_tool.copy_files_by_wildcard(
                repo_private_path + "/tools/hubitat_driver_snippets_metadata.py", repo_public_path + "/tools/examples")
            repo_tool.copy_files_by_wildcard(
                repo_private_path + "/tools/hubitat_driver_snippets_new_parser.py", repo_public_path + "/tools/examples")
            repo_tool.copy_files_by_wildcard(
                repo_private_path + "/tools/hubitat_driver_snippets.py", repo_public_path + "/tools/examples")

            # An example of a helper-include file
            repo_tool.copy_files_by_wildcard(
                repo_private_path + "/helpers/helpers-childDevices.groovy", repo_public_path + "/tools/examples/helpers")

            # Main files for the Codebuilder toolset
            repo_tool.copy_files_by_wildcard(
                repo_private_path + "/tools/hubitat_codebuilder.py", repo_public_path + "/tools")
            repo_tool.copy_files_by_wildcard(
                repo_private_path + "/tools/hubitat_hubspider.py", repo_public_path + "/tools")
            repo_tool.copy_files_by_wildcard(
                repo_private_path + "/tools/hubitat_packagemanagertool.py", repo_public_path + "/tools")
            repo_tool.copy_files_by_wildcard(
                repo_private_path + "/tools/repo_tool.py", repo_public_path + "/tools")

        log.debug("DONE updating the Hubitat-Public Repo branch \"" +
                  repo_public.head.shorthand + "\"!")
    else:
        log.warning("The Hubitat-Public Repo couldn't switch branch to \"" +
                    repo_private.head.shorthand + "\"! Aborting!")

    if(repo_tool.is_clean(repo_public)):
        log.info("The Hubitat-Public Repo is CLEAN!")
    else:
        log.warning(
            "The Hubitat-Public Repo is DIRTY! Check what needs what needs to be commited...")

    if(repo_tool.is_clean(repo_internal)):
        log.info("The Hubitat-Internal Repo is CLEAN!")
    else:
        log.warning(
            "The Hubitat-Internal Repo is DIRTY! Check what needs what needs to be commited...")
    ...
    if(len(cb.driver_new) > 0):
        log.warning(
            'These new drivers were created: \n{}'.format(cb.driver_new))
    else:
    ...
    log.info('This many PRIVATE apps were UPDATED: {}'.format(
        cb_private.app_num_updated))

    log.info('Current Default Version Number: {}'.format(
        getDriverVersion(driverVersionSpecial=cb.default_version)))
    log.info('Current Default PRIVATE Version Number: {}'.format(
        getDriverVersion(driverVersionSpecial=cb_private.default_version)))
