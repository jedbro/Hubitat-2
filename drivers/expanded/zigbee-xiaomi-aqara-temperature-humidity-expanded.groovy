...
String getDeviceInfoByName(infoName) { 
    ...
    Map deviceInfo = ['name': 'Zigbee - Xiaomi/Aqara Temperature & Humidity Sensor', 'namespace': 'oh-lalabs.com', 'author': 'Markus Liljergren', 'filename': 'zigbee-xiaomi-aqara-temperature-humidity', 'importUrl': 'https://raw.githubusercontent.com/markus-li/Hubitat/development/drivers/expanded/zigbee-xiaomi-aqara-temperature-humidity-expanded.groovy']
...
ArrayList<String> parse(String description) {
    ...
    if(hasCorrectCheckinEvents(maximumMinutesBetweenEvents=90) == false) {
        List<String> restoreCmd = zigbeeReadAttribute(CLUSTER_BASIC, 0x0005, [:], delay=70)
        logging("Restoring bind settings", 100)
        restoreCmd += ["zdo bind ${device.deviceNetworkId} 0x01 0x01 0x0000 {${device.zigbeeId}} {}", "delay 70",]
        restoreCmd += ["zdo bind ${device.deviceNetworkId} 0x01 0x01 0x0402 {${device.zigbeeId}} {}", "delay 70",]
        restoreCmd += ["zdo bind ${device.deviceNetworkId} 0x01 0x01 0x0403 {${device.zigbeeId}} {}", "delay 70",]
        restoreCmd += ["zdo bind ${device.deviceNetworkId} 0x01 0x01 0x0405 {${device.zigbeeId}} {}", "delay 70",]
