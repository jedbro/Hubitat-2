...
class HubitatPackageManagerPackage:
    ...
    def applyDefaults(self, minimumHEVersion=None, author=None, dateReleased=None):
        ...
        if(self.manifestDict['dateReleased'] == None):
            if(dateReleased == None):
                self.manifestDict['dateReleased'] = date.today().strftime(
                    '%Y-%m-%d')
            else:
    ...
    def addFile(self, name, version, location, id=None):
        newFile = {
            "id": id,
            "name": name,
            "version": version,
        ...
        if(self.isBeta == False):
            del newFile["betaVersion"]
            del newFile["betaLocation"]
        else:
            del newFile["version"]
            del newFile["location"]

        self.manifestDict['files'].append(newFile)
    ...
    def _extractSavedManifest(self, savedManifest):
        ...
        if(os.path.isfile(savedManifest) and os.access(savedManifest, os.R_OK)):
            ...
            if('files' in mdata):
                files_dict = {}
                # Extract the driver data from the manifest and store it based on the UUID
                for d in mdata['files']:
                    files_dict[d['name']] = d
                for d in self.manifestDict['files']:
                    # Make sure we don't have fields that don't belong
                    if(self.isBeta == False):
                        if "betaVersion" in d:
                            del d["betaVersion"]
                        ...
                        if "location" in d:
                            del d["location"]
                    if(d['name'] in files_dict and files_dict[d['name']]['id'] != None):
                        d['id'] = files_dict[d['name']]['id']
    ...
    def buildManifest(self, output="packageManifest.json", extraInput=None):
        ...
        for d in self.manifestDict['drivers']:
            if(d['id'] == None):
                d['id'] = str(uuid.uuid5(uuid.NAMESPACE_DNS,
                                         d['name'] + d['namespace']))
